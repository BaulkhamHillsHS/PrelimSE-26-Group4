...
class Login(ctk.CTk):
    ...
    def _build_frame(self):
        ...
        self.entry_username = ctk.CTkEntry(self.frame_input, width = 300, fg_color=FRAME,
text_color=TEXT,
border_color=BORDER,
...
placeholder_text="Username or email")
        ...
        self.entry_password = ctk.CTkEntry(self.frame_input, width = 300, fg_color=FRAME,
text_color=TEXT,
border_color=BORDER,
placeholder_text_color=SUBTEXT,
...
text_color=TEXT,
border_color=BORDER,
placeholder_text_color=SUBTEXT, placeholder_text="2FA code")
...
class HomePage(ctk.CTk):
    ...
    def _build_frame(self):
        ...
        self.frame_input = ctk.CTkFrame(self, fg_color=FRAME) #240E07 #9C9C9C
        self.grid_rowconfigure(0, weight=1)
        self.grid_columnconfigure(0, weight=1)
        self.frame_input.grid(row=0, column=0)
...
text_color=TEXT,
        ...
        self.search_entry = ctk.CTkEntry(self.frame_input, width=300, fg_color=FRAME,
text_color=TEXT,
border_color=BORDER,
placeholder_text_color=SUBTEXT, placeholder_text="Search...")
    ...
    def run_search(self):
        ...
        for i, watch in enumerate(watches2):
            if query.lower().replace(" ", "").replace("-", "") in watch: #eliminating spaces for ease of search and ignoring typos (with spaces) etc.
                self.search_results_button = ctk.CTkButton(self.frame_input, text=watches[i],fg_color=PRIMARY,
hover_color=PRIMARY_DARK,
text_color=TEXT, command=lambda m=watches[i]: self.play_movie(m))
        ...
        if searchMatches == 3:
            ctk.CTkLabel(self.frame_input, text="No results found.", text_color=TEXT).grid(row=3, column=0, padx=20, pady=10)
    ...
    def subscription_details(self):
        ...
        self.frame_input = ctk.CTkFrame(self, fg_color=FRAME)
        self.frame_input.grid(row=0, column=0, sticky="nsew")
        home_btn = ctk.CTkButton(self.frame_input, text="SoggyStreams", font = ("Comic Sans MS", 24, "bold"), fg_color=PRIMARY, hover_color=PRIMARY_DARK, command=self.return_home)
        home_btn.grid(row=0, column=0, padx=20, pady=20, sticky = "nw")
        ...
        ctk.CTkButton(self.frame_input, text="Reveal Password", font=("Comic Sans MS", 14),fg_color=PRIMARY,
hover_color=PRIMARY_DARK,
text_color=TEXT, command=self.decrypt_password).grid(row=4, column=2, padx=20, pady=10, sticky="w")
        ctk.CTkButton(self.frame_input, text="Change Password", font=("Comic Sans MS", 14),fg_color=PRIMARY,
hover_color=PRIMARY_DARK,
text_color=TEXT, command=self.change_password_page).grid(row=4, column=3, padx=20, pady=10, sticky="w")
        ...
        ctk.CTkButton(self.frame_input, text="Change Plan", font=("Comic Sans MS", 14),fg_color=PRIMARY,
hover_color=PRIMARY_DARK,
text_color=TEXT, command=self.change_plan_page).grid(row=5, column=2, padx=20, pady=10, sticky="w")
    ...
    def change_password_page(self):
        ...
        self.confirmnumberchange = ctk.CTkButton(self.frame_input, text="Confirm",font=("Comic Sans MS", 14),fg_color=PRIMARY,
hover_color=PRIMARY_DARK,
text_color=TEXT, command=self.checkpasswords)
    ...
    def manage_profiles(self):
        ...
        for i, profile_typ in enumerate (self.user_proftyp_lst):
            ctk.CTkLabel(self.frame_input, text=profile_typ, text_color=TEXT, font=("Comic Sans MS", 14)).grid(row=i+2, column=1, padx=10, pady=10)
    ...
    def editprofiles(self):
        ...
        for i, profile in enumerate (self.user_profile_lst):
            ...
            ctk.CTkButton(self.frame_input, text=profile, font=("Comic Sans MS", 14), fg_color=PRIMARY,
hover_color=PRIMARY_DARK,
text_color=TEXT, command=self.profileclicked).grid(row=i+2, column=1, padx=10, pady=10)
        ...
        with open('userprofiles.csv', 'r') as csv_file:
            ...
            for row in proftypreader:
                if row ["username"] == self.user_logged_in:
                    self.user_proftyp_lst.append(row['profile_type'])
        for i, profile_typ in enumerate (self.user_proftyp_lst):
            ctk.CTkLabel(self.frame_input, text=profile_typ, text_color=TEXT, font=("Comic Sans MS", 14)).grid(row=i+2, column=2, padx=10, pady=10)
                
        delte_prof_btn = ctk.CTkButton(self.frame_input, text="Delete",  fg_color=PRIMARY,
                                        hover_color=PRIMARY_DARK, text_color=TEXT, command=self.checkbox_event)
        delte_prof_btn.grid(row=7, column=1, padx=10, pady=10)
        ctk.CTkButton(self.frame_input, text="Back", fg_color=PRIMARY, hover_color=PRIMARY_DARK, text_color=TEXT, command=self.manage_profiles).grid(row=8, column=1, padx=10, pady=10) 
...
class Signup:
    ...
    def _build_frame(self):
        ...
        self.textbox = ctk.CTkEntry(self.frame_input, placeholder_text="Enter", show="*", fg_color=FRAME, text_color=TEXT, border_color=BORDER, placeholder_text_color=SUBTEXT)
